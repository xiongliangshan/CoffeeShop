...
public class ProgressPercent extends View {
    ...
    public ProgressPercent(Context context, @Nullable AttributeSet attrs) {
        ...
        mUnPassColor = a.getColor(R.styleable.ProgressPercent_unpassColor,Color.GRAY);
        a.recycle();
        initDraw();
    }
    ...
    public void updateProgress(long startTime,long endTime){
        ...
        LogUtil.d(TAG,"status = "+status+"|pass ="+passMinute+"|unpass="+unPassMinute+"|over ="+overMinute);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int baseLineY = (int) (paddingTop + height/2 - top/2 - bottom/2);//基线中间点的y轴计算公式
        textPaint.setColor(Color.BLACK);
        canvas.drawText(mStartText,paddingLeft+2,baseLineY,textPaint);
        canvas.drawText(mEndText,getWidth()-paddingRight-endTextWidth-2,baseLineY,textPaint);
        RectF progressRect = new RectF(paddingLeft+startTextWidth+4,paddingTop,getWidth()-paddingRight-endTextWidth-4,height);
        if(status==-1){
            //未开始
            ...
            canvas.drawRoundRect(progressRect,20,20,rectPaint);
            textPaint.setColor(Color.BLACK);
            float beforeStartWidth = textPaint.measureText(mBeforeStartText);
            canvas.drawText(mBeforeStartText,progressRect.left+(progressRect.width()-beforeStartWidth)/2.0f,baseLineY,textPaint);
        }else if(status==0){
            //进行中
            RectF rectPass = new RectF(progressRect.left+1,progressRect.top+1,progressRect.width()*passMinute/(passMinute+unPassMinute)-1,progressRect.bottom-1);
            RectF rectUnPass = new RectF(paddingLeft+width-rectPass.right+1,paddingTop+1,width-1,height-1);
            ...
            canvas.drawRoundRect(rectPass,20,20,rectPaint);

            float passTextWidth = textPaint.measureText(String.valueOf(passMinute));
            textPaint.setColor(Color.BLACK);
            canvas.drawText(String.valueOf(passMinute),rectPass.left+(rectPass.width()-passTextWidth)/2.0f,baseLineY,textPaint);


            float unPassTextWidth = textPaint.measureText(String.valueOf(unPassMinute));
            canvas.drawText(String.valueOf(unPassMinute),rectUnPass.left+(rectUnPass.width()-unPassTextWidth)/2.0f,baseLineY,textPaint);
        }else{
            //已超时
            rectPaint.setStyle(Paint.Style.FILL_AND_STROKE);
            rectPaint.setColor(Color.RED);
            canvas.drawRoundRect(progressRect,24,24,rectPaint);
            textPaint.setColor(Color.WHITE);
            float overTextWidth = textPaint.measureText(mOverText);
            canvas.drawText(mOverText,progressRect.left+(progressRect.width()-overTextWidth)/2.0f,baseLineY,textPaint);
        }
    }
}
